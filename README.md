...
グラフのノードや「実行」ページの ▶ を押すと、**ページ内のモーダルで入力→実行→進捗→結果表示まで完結**します。
裏側では GitHub API で `▶ run:` Issue が作成され、`run-agent.yml` が実行して Issue をクローズ+結果をコメントし、ページがそれをポーリングして表示します。

- **初回のみ**: [Fine-grained トークン](https://github.com/settings/personal-access-tokens/new)を作成(Repository access: AgAg のみ / Permissions: **Issues = Read and write** のみ)して、▶ を押すと出る設定画面に貼り付け。トークンはブラウザの localStorage にのみ保存され、GitHub API 以外へは送信されません
- **オーナー(あなた)が起票した `▶ run:` で始まる Issue のみ実行されます**(第三者の Issue ではワークフローが起動しません)
- サブエージェント(`agent/agents/*.md`)に加えて**スキルも実行可能**(`skill:<名前>` — スキルを入力に適用)
- 日次エージェントが成長アクションとして新しいサブエージェント/スキルを増やしていきます
